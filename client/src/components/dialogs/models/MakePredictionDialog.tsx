...
import { CircularProgress, Dialog, DialogTitle, DialogContent, Typography,
        ...
        InputLabel, Box, DialogActions, Divider } from '@mui/material'
...
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, 
    ResponsiveContainer, TooltipProps, ReferenceLine, Label,
    BarChart, Bar, Legend } from 'recharts'

// ** Dropdown data
import { regimes, containerKinds, containerTypes, firstServiceNames, teus,
        firstLineKeys, lastLineKeys, dischargeLines, departurePorts, loadPorts,
        loadLines, goods, companies, fieldType, regimesMap, tdayDistribution, gumrukDistribution } from './containerFieldData'
...
const MakePredictionDialog = (props: IProps) => {
    ...
    const mlPrediction = async () => {
        ...
        //axios.get("http://localhost:7777/").then(res=>console.log(res)).catch(err=>console.log("err", err))
        let url = ""
        ...
        if(url.length){
            axios.post(url, containerData, {
              ...
              .then(function (response) {
                console.log("ml result: ", response);
    ...
    let checkPredictionForm = (): boolean | undefined => {
        if((regime && regime.length) && (containerKind && containerKind.length) && (containerType && containerType.length) && 
            (firstServiceName && firstServiceName.length) && firstLineKey && lastLineKey && 
            (dischargeLine && dischargeLine.length) && (loadLine && loadLine.length) && (good && good.length) && (company && company.length)){
                return false
        }
        return true
    }
    ...
    return (
        <div>
            <Dialog onClose={()=> {handleClose(); setWaitingRequest(false); setShowingResult(false)}} aria-labelledby='customized-dialog-title' open={open} fullWidth maxWidth='md'>
                <DialogTitle id='customized-dialog-title' sx={{ p: 4 }}>
                    ...
                    <IconButton
                        aria-label='close'
                        onClick={()=> {handleClose(); setWaitingRequest(false); setShowingResult(false)}}
                                ...
                                <Grid item xs={12} sm={4}>
                                    <FormControl fullWidth>
                                        <Autocomplete
                                            ...
                                            value={regime}
                                            onChange={(e, value) =>{console.log("regime:", value);setRegime(value)}}
                                            renderInput={(params) => (
                                                <StyledTextField
                                                  {...params}
                                                  label={"Regime"}
                                ...
                                <Grid item xs={12} sm={4}>
                                    <FormControl fullWidth>
                                        <Autocomplete
                                            ...
                                            value={containerKind}
                                            onChange={(e,value) =>{setContainerKind(value)}}
                                            renderInput={(params) => (
                                                <StyledTextField
                                                  {...params}
                                ...
                                <Grid item xs={12} sm={4}>
                                    <FormControl fullWidth>
                                        <Autocomplete
                                            ...
                                            onChange={(e,value) =>{setFirstServiceName(value)}}
                                            renderInput={(params) => (
                                                <StyledTextField
                                                  {...params}
                                ...
                                <Grid item xs={12} sm={4}>
                                    <FormControl fullWidth>
                                        <Autocomplete
                                            ...
                                            value={firstLineKey}
                                            onChange={(e,value) =>{setFirstLineKey(value)}}
                                            renderInput={(params) => (
                                                <StyledTextField
                                                  {...params}
                                ...
                                <Grid item xs={12} sm={4}>
                                    <FormControl fullWidth>
                                        <Autocomplete
                                            ...
                                            value={lastLineKey}
                                            onChange={(e,value) =>{setLastLineKey(value)}}
                                            renderInput={(params) => (
                                                <StyledTextField
                                                  {...params}
                                                  label={"Last Line Key"}
                                ...
                                <Grid item xs={12} sm={4}>
                                    <FormControl fullWidth>
                                        <Autocomplete
                                            ...
                                            value={dischargeLine}
                                            onChange={(e,value) =>{setDischargeLine(value)}}
                                            renderInput={(params) => (
                                                <StyledTextField
                                                  {...params}
                                ...
                                <Grid item xs={12} sm={4}>
                                    <FormControl fullWidth>
                                        <Autocomplete
                                            ...
                                            value={departurePort}
                                            onChange={(e,value) =>{setLastLineKey(departurePort)}}
                                        />
                                    </FormControl>
                                </Grid>
                                <Grid item xs={12} sm={4}>
                                    <FormControl fullWidth>
                                        <Autocomplete
                                            ...
                                            value={loadPort}
                                            onChange={(e,value) =>{setLoadPort(value)}}
                                        />
                                    </FormControl>
                                </Grid>
                                <Grid item xs={12} sm={4}>
                                    <FormControl fullWidth>
                                        <Autocomplete
                                            ...
                                            value={loadLine}
                                            onChange={(e,value) =>{setLoadLine(value)}}
                                            renderInput={(params) => (
                                                <StyledTextField
                                                  {...params}
                                                  label={"Load Line"}
                                ...
                                <Grid item xs={12} sm={4}>
                                    <FormControl fullWidth>
                                        <Autocomplete
                                            ...
                                            value={good}
                                            onChange={(e,value) =>{setGood(value)}}
                                            renderInput={(params) => (
                                                <StyledTextField
                                                  {...params}
                                                  label={"Good"}
                        ...
                        <Box sx={{ display: 'flex', justifyContent: "center" }}>
                                ...
                                <Grid container>
                                    <Grid item xs={12}>
                                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: "center"}}>
                                            <Typography variant='h6' sx={{  mr: 1.0 }}>Prediction Result:</Typography>
                                            <Typography variant='h5' sx={{ mr: 1.5, color: parseFloat(predictionResult)>=10 ?  "#FF4C51" : "#56CA00" }}>
                                    ...
                                    <Grid item xs={12}>
                                        <RechartsWrapper>
                                            <Box sx={{ height: 350, color: "black" }}>
                                                <ResponsiveContainer>
                                                    <AreaChart height={350} width={800} data={tdayDistribution} 
                                                        margin={{top: 10,right: 30,left: 0,bottom: 0}}>
                                                        <CartesianGrid  opacity={0.5}/>
                                                        <XAxis dataKey='tday' label={{fill: 'white'}}/>
                                                        <YAxis  label={{fill: 'white'}}/> 
                                                        <Tooltip content={CustomLineTooltip}/> 
                                                        <Area type="monotone" name="container count" dataKey="count" stroke="#8884d8"  fill='rgb(115, 103, 240)' />
                                                        <Legend />
                                                        <ReferenceLine 
                                                        ...
                                                        >
                                                            <Label 
                                                                ...
                                                                style={{ fontSize: '80%', fill: parseFloat(predictionResult)>=10 ?  "#FF4C51" : "#56CA00" }}
                                                            />
                                                        </ReferenceLine>
                                                    </AreaChart>
                                                </ResponsiveContainer>
                           ...
                           {model.modelJob === "gumruk" && 
                                <Grid container>
                                    <Grid item xs={12}>
                                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: "center"}}>
                                            <Typography variant='h5' sx={{ mr: 1.5, color: parseInt(predictionResult) ? "#FF4C51" : "#56CA00" }}>
                                    ...
                                    <Grid item xs={12}>
                                        <RechartsWrapper>
                                            <Box sx={{ height: 350, color: "black" }}>
                                                <ResponsiveContainer>
                                                    <BarChart height={350} width={800} data={gumrukDistribution}
                                                        barSize={60} margin={{top: 10,right: 30,left: 10,bottom: 0}}>
                                                        <CartesianGrid opacity={0.5} />
                ...
                <DialogActions sx={{ pt: 0, display: 'flex', justifyContent: 'center' }}>
                    <Box className='demo-space-x'>
                        {(!waitingRequest && !showingResult) &&
                            <Button size='large' type='submit' variant='contained' onClick={mlPrediction} disabled={checkPredictionForm()}>
                                Predict
                            </Button>
                        }
                        {(!waitingRequest && showingResult) && 
                            <Button size='large' type='submit' variant='contained' onClick={()=>{setWaitingRequest(false); setShowingResult(false)}}>
                                Go Back
                            </Button>
                        }
                    </Box>
