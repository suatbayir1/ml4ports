...
const ApexBarChartCompany = () => {
  ...
  useDebouncedEffect(
    ...
    500,
    [selectedGoodKey, startDate, endDate, selectedAnnotationOption]
  )

  const getUniqueCompanyCategories = flattenedData => {
    const companyCategories = new Set(flattenedData.map(item => item.COMPANY_CAT))
  ...
  const getYearsInRange = (start, end) => {
    ...
    if (end != undefined) endYear = end.getFullYear()
