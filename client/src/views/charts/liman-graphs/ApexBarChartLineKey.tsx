...
const ApexBarChartLineKey = () => {
      ...
      const fetchData = async () => {
        try {
          const response = await fetch(
            getUrl(
              `container/grouped-line?selectedLineKey=${selectedLineKey}&startDate=${formattedStartDate}&endDate=${formattedEndDate}`
            )
          )
          if (response.ok) {
  ...
  return (
    <Card>
      ...
      <CardContent>
        <FormControlLabel
          control={<Checkbox checked={showAnnotations} onChange={event => setShowAnnotations(event.target.checked)} />}
