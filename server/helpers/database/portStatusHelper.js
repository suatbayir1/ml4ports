...
const getCurrentPortStatus = asyncErrorWrapper(async () => {
  try {
    ...
    const result = await pool.request().query(getCurrentPortStatusQuery());
    ...
    const obj = {
      total_cont_count: result.recordset.length,
      summary: groupedData,
...
const getDailyContainersStatus = asyncErrorWrapper(async (date) => {
  try {
    const pool = await poolPromise;
    const result = await pool.request().query(getDailyContainersQuery(date));
